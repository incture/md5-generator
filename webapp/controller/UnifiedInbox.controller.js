sap.ui.define([
	...
	"sap/ui/core/Popup"
], function (BaseController, formatter, taskManagement, workbox, utility, JSONModel, Dialog, ExtDatePicker, Popup) {
	"use strict";
	...
	return BaseController.extend("oneapp.incture.workbox.controller.UnifiedInbox", {
		...
		createSubTaskFnOld: function () {
			...
			/*	this.doAjax("/WorkboxJavaService/idpMapping/getUsers", "GET", null, function (oData) {
			...
			this.doAjax("/WorkboxJavaService/customProcess/getProcess?processType=All", "GET", null, function (oData) {
		...
		// for generic sub task creation
		createSubTaskFn: function () {
			if (!this._subTaskTemplateContent) {
				this._subTaskTemplateContent = sap.ui.xmlfragment("oneapp.incture.workbox.fragment.CreateNewTask", this);
			}
			var oCustomTaskModel = this.getModel("oCustomTaskModel");
			this.doAjax("/WorkboxJavaService/customProcess/getProcess?processType=Ad-Hoc", "GET", null, function (oData) {
					oCustomTaskModel.setProperty("/customProcessNames", oData.processDetails);
					oCustomTaskModel.refresh();
				}.bind(this),
				function (oEvent) {}.bind(this));
			var taskObjectDetails = this.getModel("oTaskInboxModel").getProperty("/lineItemTaskDetails");
			var url = "/WorkboxJavaService/detailPage/dynamicDetails?taskId=" + taskObjectDetails.taskId;
			var template = {
				"actionType": "Submit",
				"isEdited": null,
			...
			this.doAjax(url, "GET", null, function (oData) {
					for (var k = 0; k < oData.headerDto.length; k++) {
						if (oData.headerDto[k].key === "Description") {
							template.listOfProcesssAttributes[0].customAttributeTemplateDto[0].value = oData.headerDto[k].value;
							break;
						}
					}
					...
					for (var i = 0; i < oData.contentDto.length; i++) {
						if (oData.contentDto[i].dataType === "TASKDETAILS") {
							contentDto = oData.contentDto[i].contentDto
							for (var j = 0; j < contentDto.length; j++) {
								var tempCA = jQuery.extend(true, {}, customAttributeTemplateDto);
								tempCA = this.getSubTaskContent(tempCA, contentDto[j]);
								if (taskObjectDetails.processName === "CourseRegistrationDemo" || taskObjectDetails.processName ===
									"ReimbursementofregisteredcoursesDemo") {
									if (tempCA.label.toLocaleLowerCase() === "total cost") {
										tempTotal = tempTotal + parseInt(tempCA.value, 10);
										itotalCost = template.listOfProcesssAttributes[0].customAttributeTemplateDto.length;
									}
								}
								template.listOfProcesssAttributes[0].customAttributeTemplateDto.push(tempCA);

							}
						} else if (oData.contentDto[i].dataType === "LINEITEM") {
							// Add table control for subtask
							contentDto = oData.contentDto[i].contentDto
							var tempCA = jQuery.extend(true, {}, customAttributeTemplateDto);
							if (contentDto.length > 0) {
								tempCA = this.getSubTaskContent(tempCA, contentDto[0]);
								tempCA.tableAttributes = null;
								tempCA.dataType = "TABLE";
								tempCA.tableContents = [];
								for (var j = 0; j < contentDto.length; j++) {
									if (contentDto[j].attributeValues.length > 0) {
										var tableAttributes = []
										for (var z = 0; z < contentDto[j].attributeValues.length; z++) {
											var attrValue = contentDto[j].attributeValues[z];
											var tableContents = jQuery.extend(true, {}, customAttributeTemplateDto);
											tableContents = this.getSubTaskContent(tableContents, contentDto[j])
											tableContents.key = attrValue.key;
											tableContents.label = attrValue.key;
											tableContents.value = attrValue.attributeValue;
											tableContents.isRunTime = false;
											tableContents.dataType = "INPUT";
											tableContents.processName = taskObjectDetails.processName;
											if (attrValue.key.toLowerCase().includes("date") && (this.oAppModel.getProperty("/taskObjectDetails/processName") ===
													"ReimbursementofregisteredcoursesDemo" || this.oAppModel
													.getProperty(
														"/taskObjectDetails/processName") === "CourseRegistrationDemo")) {
												tableContents.dataType = "DATETYPE";
											}
											tableContents.origin = "Table";
											tableContents.isMandatory = contentDto[j].isMandatory;
											tableContents.isEditable =
												contentDto[j].isEditable;
											tableContents.isActive = contentDto[j].isActive;
											tableAttributes.push(tableContents);
											if (taskObjectDetails.processName === "CourseRegistrationDemo" || taskObjectDetails.processName ===
												"ReimbursementofregisteredcoursesDemo") {
												if (tableContents.label.toLocaleLowerCase() === "tuition fees") {
													tempTotal = tempTotal + parseInt(tableContents.value, 10);
												}
											}
										}
										tempCA.tableContents.push({
						...
						} else if (oData.contentDto[i].dataType === "ATTACHMENT") {
							// var savedAttachment = oTaskDetailModel.getProperty("/attachmentValues");
							attachmentContent = oData.contentDto[i].contentDto;
							for (var j = 0; j < oData.contentDto[i].contentDto.length; j++) {
								var tempAttachment = jQuery.extend(true, {}, attachmentDto);
								tempAttachment.isEditable = attachmentContent[j].isEditable;
								tempAttachment.dataType = "INPUT";
								tempAttachment.isVisible = attachmentContent[j].isVisible;
								tempAttachment.isActive = attachmentContent[j].isActive;
								tempAttachment.isMandatory = attachmentContent[j].isMandatory;
								tempAttachment.label = attachmentContent[j].label;
								tempAttachment.processName = taskObjectDetails.processName;

								if (attachmentContent[j].attributeValues.length > 0) {
									tempAttachment.value = JSON.stringify(attachmentContent[j].attributeValues); //cross check with java
								}
								if (attachmentContent[j].attributePath) {
									tempAttachment.key = attachmentContent[j].attributePath.slice(2, attachmentContent[j].attributePath.length - 1);
								} else {
									tempAttachment.key = JSON.stringify(attachmentContent[j].attributeValues); //cross check with java
								}
					...
					if (itotalCost) {
						// oData.contentDto[itotalCost[0]].contentDto[itotalCost[1]].value = tempTotal
						template.listOfProcesssAttributes[0].customAttributeTemplateDto[itotalCost].value = tempTotal;

					}
					oCustomTaskModel.setProperty("/customInstance", template);
					this._subTaskTemplateContent.addStyleClass("wbSubTaskVBoxHeight")
					taskManagement.uploadInstancesTM(this, template, ExtDatePicker, oCustomTaskModel);
				}.bind(this),
				function (oError) {}.bind(this));
			oCustomTaskModel.setProperty("/selectedProcess", taskObjectDetails.processName);
		...
		getSubTaskContent: function (tempCA, contentDto) {

			if (contentDto.attributePath) {
				tempCA.key = contentDto.attributePath.slice(2, contentDto.attributePath.length - 1);
			} else {
				tempCA.key = contentDto.key;
			}
			tempCA.isEditable = contentDto.isEditable;
			tempCA.isActive = contentDto.isActive;
			tempCA.dataType = contentDto.dataType;
			tempCA.runTimeType = contentDto.runTimeType;
			tempCA.dataType = contentDto.dataType;
			tempCA.label = contentDto.label;
			tempCA.isRunTime = contentDto.isRunTime;
			if (tempCA.isRunTime) {
				tempCA.dataTypeKey = 1;
				tempCA.dropDownType = contentDto.runTimeType;
				tempCA.url = contentDto.url;
				var arr = [];
				tempCA.valueList = [];
				if (typeof (contentDto.value) === "string") {
					arr = (contentDto.value).split(",");
				} else {
					arr = contentDto.value;
				}
				for (var z = 0; z < arr.length; z++) {
					var obj = {
						"id": arr[z],
						"type": contentDto.runTimeType
					};
					tempCA.valueList.push(obj);
				}
			} else {
				tempCA.value = contentDto.value;
			}
			if (tempCA.dataType === "DROPDOWN") {
				tempCA.url = contentDto.url;
				tempCA.value = contentDto.value;
				taskManagement.fetchDropdownList(this, tempCA);
			}
			tempCA.isMandatory = contentDto.isMandatory;

			return tempCA;

		},
		dynamicInstanceCreation: function (oEvent) {
			var bMergeSubTaskData = true;
			var selectedItem = oEvent.getSource().getSelectedItem().getKey();
			taskManagement.dynamicInstanceCreationTM(this, selectedItem, ExtDatePicker, bMergeSubTaskData);
		},
		...
		subTaskCreationPostCall: function () {
			...
			if (!validationCheck) {
				...
				this.doAjax("/WorkboxJavaService/tasks/createTasks", "POST", data, function (oData) {
		...
		getTaskBoardData: function () {
			...
			// If view is not applied remove filters
			if (!this.oAppModel.getProperty("/isViewApplied")) {
				this.clearFilters();
				this.removeAllTokens(false);
			} else {
				// If view is applied retain filters
				inboxType = this.getModel("oAdvanceFilterModel").getProperty("/searchInboxType");
				var viewPayload = JSON.parse(this.getModel("oAdvanceFilterModel").getProperty("/viewAppliedContext/viewPayload"));
				var filterData = {
					advanceFilter: {
						advanceSearch: viewPayload.advanceSearch,
						filterMap: viewPayload.filterMap
					},
					inboxType: inboxType,
					page: page,
					quickFilter: {}
				};
				var parsedFilters = this.getFilterSearchFilters(filterData, []);
				this.oAppModel.setProperty("/filterTokens", parsedFilters.filterTokens);
				this.oAppModel.setProperty("/filterData", parsedFilters.filterData);
			}
			var oTaskInboxModel = this.getModel("oTaskInboxModel");

			var url = "/WorkboxJavaService/inbox/filterService?inboxType=" + inboxType + "&page=" + page;
			...
			this.doAjax(url, "GET", null, function (oData) {
				...
				// show only relevant tray
				if (this.oAppModel.getProperty("/isViewApplied")) {
					var trayList = [];
					for (var i = 0; i < oData.workboxStoryBoardDto.length && oData.workboxStoryBoardDto[i].filterData; i++) {
						if (this.getModel("oAdvanceFilterModel").getProperty("/viewAppliedContext/viewPayload") === oData.workboxStoryBoardDto[i].filterData) {
							trayList.push(oData.workboxStoryBoardDto[i]);
						}
					}
					oTaskInboxModel.setProperty("/taskBoardDto", trayList);
					this.oAppModel.setProperty("/taskBoardPageCount", trayList.length);
				}
				oTaskInboxModel.refresh(true);
				this.oAppModel.refresh(true);
				this.pagination();
		...
		loadTrayTasksFn: function (oEvent) {
			...
			filterData.advanceFilter.advanceSearch = advFilter.advanceSearch;
		...
		onClickFilterDetail: function () {
			...
			if (oAppModel.getProperty("/graphClicked") && !oAppModel.getProperty("/removeFilterToken")) {
				// If task board is open from we click on graph and navigate to inbox, change it to card view
				if (oAppModel.getProperty("/WB_TASK_BOARD_VIEW")) {
					oAppModel.setProperty("/selectedInboxView", "WB_CARD_VIEW");
					oAppModel.setProperty("/WB_TASK_BOARD_VIEW", false)
					oAppModel.setProperty("/WB_CARD_VIEW", true)
				}

				if (oAppModel.getProperty("/dynamicGraphFilterPayload")) {
					filterData = oAppModel.getProperty("/dynamicGraphFilterPayload")
				} else {
					oAppModel.setProperty("/isAppliedAdvancedFilter", false);
					filterData.inboxType = "AdminInbox";
					this.setCurrentPage(null, "AdminInbox", null, false, true);
					if (oAppModel.getProperty("/graphProcessName")) {
						if (oAppModel.getProperty("/graphProcessName") !== "ALL") {
							filterData.quickFilter.processName = oAppModel.getProperty("/graphProcessName");
						}
					}
					if (oAppModel.getProperty("/graphYear")) {
						filterData.quickFilter.duration = oAppModel.getProperty("/graphYear");
						oAppModel.setProperty("/graphMeasureNames", "taskCompletionTrendList");
					}
					if (oAppModel.getProperty("/graphMeasureNames")) {
						if (oAppModel.getProperty("/graphMeasureNames") === "In Time" || oAppModel.getProperty("/graphMeasureNames") ===
							"On Time") {
							filterData.quickFilter.freeFilter = "On Time";
						} else if (oAppModel.getProperty("/graphMeasureNames") === "Critical") {
							filterData.quickFilter.freeFilter = "Critical";
						} else if (oAppModel.getProperty("/graphMeasureNames") === "SLA Breached") {
							filterData.quickFilter.freeFilter = oAppModel.getProperty("/graphMeasureNames");
						} else {
							if (oAppModel.getProperty("/graphMeasureNames") !== "taskCompletionTrendList") {
								filterData.quickFilter.freeFilter = oAppModel.getProperty("/graphMeasureNames");
							}
						}
					}
					if (oAppModel.getProperty("/graphProcessDisplayName")) {} else {
						if (oAppModel.getProperty("/graphStatus")) {
							if (oAppModel.getProperty("/graphStatus") !== "NOT COMPLETED") {
								filterData.quickFilter.status = oAppModel.getProperty("/graphStatus");
							}
						}
					}
					if (!filterData) {
					...
					filterData.page = oAppModel.getProperty("/currentPage");
					if (oAppModel.getProperty("/graphUser")) {
						filterData.quickFilter.userId = oAppModel.getProperty("/graphUser");
					}
					if (oAppModel.getProperty("/graphMeasureDisplayName") === "My Tasks") {
						filterData.quickFilter.userId = oAppModel.getProperty("/loggedInUserDetails/userId");
					}
					if (oAppModel.getProperty("/graphName") === "userWorkItemCount") {
						if (oAppModel.getProperty("/graphProcessMeasureName") === "In Time" || oAppModel.getProperty("/graphProcessMeasureName") ===
							"On Time") {
							filterData.quickFilter.freeFilter = "On Time";
						} else if (oAppModel.getProperty("/graphProcessMeasureName") === "Critical") {
							filterData.quickFilter.freeFilter = "Critical";
						} else if (oAppModel.getProperty("/graphProcessMeasureName") === "SLA Breached") {
							filterData.quickFilter.freeFilter = oAppModel.getProperty("/graphMeasureNames");
						}
					}

					if (filterData.quickFilter.status) {
						if (filterData.quickFilter.status.includes("COMPLETED") || filterData.quickFilter.status.includes("Completed")) {
							this.setCurrentPage(null, "CompletedTasks", "Completed Tasks", true, true);
			...
			if (oAppModel.getProperty("/currentViewPage") === "CompletedTasks" && !filterData.sortingDtos) {
				filterData.sortingDtos = [{
					"orderBy": "completedAt",
					"orderType": "DESC"
				}];
			...
			url = "/WorkboxJavaService/inbox/filterService";
		...
		getAllTasks: function (filterData, url) { //	getting the filtered data, url to get all tasks
			...
			this.doAjax(url, "POST", filterData, function (oEvent) {
				...
				if (this.oAppModel.getProperty("/inboxType") === "Draft" && this.oAppModel.getProperty("/WB_CALENDAR_VIEW")) {
					this.oAppModel.setProperty("/WB_CARD_VIEW", true);
					this.oAppModel.setProperty("/WB_CALENDAR_VIEW", false);
					this.oAppModel.setProperty("/selectedInboxView", "WB_CARD_VIEW");
				}
		...
		// generate tokens to show on the search field, combine the sort, quick filter and advance filter
		getFilterSearchFilters: function (filterData, filterTokens) {
			var oAppModel = this.oAppModel;
			if (oAppModel.getProperty("/sortingDto")) {
				if (oAppModel.getProperty("/sortingDto").length > 0) {
					filterData.sortingDtos = oAppModel.getProperty("/sortingDto");
					var sort = oAppModel.getProperty("/sortingDto")[0];
					var obj = {
						"key": sort.orderBy,
						"value": sort.orderBy + " : " + sort.orderType,
						"type": "sortingDto",
					};
					filterTokens.push(obj);
				}
			}
			var keys = Object.keys(filterData.quickFilter);
			var values = Object.values(filterData.quickFilter);

			if (filterData.quickFilter && !!keys.length) {
				for (var i = 0; i < keys.length; i++) {
					if (values[i] !== undefined) {
						var obj = {};
						obj.key = keys[i];
						obj.value = values[i];
						obj.type = "quickFilter";
						filterTokens.push(obj);
					}
				}
			}
			...
			if (oAppModel.getProperty("/isAppliedAdvancedFilter")) {
				...
				for (var i = 0; i < advanceFilter.length; i++) {
					if (advanceFilter[i].value.includes(",") && advanceFilter[i].value.split(",").length > 1 && advanceFilter[i].filterType ===
						"MULTICOMBOBOX") {
						var splitValue = "";
						if (advanceFilter[i].displayValue) {
							splitValue = advanceFilter[i].displayValue.split(",");
						} else if (advanceFilter[i].value) {
							advanceFilter[i].value = advanceFilter[i].value.replace(advanceFilter[i].label + ":", "").trim();
							splitValue = advanceFilter[i].value.split(",");
						}
						for (var j = 0; j < splitValue.length; j++) {
							var copyStandardFilter = Object.assign({}, advanceFilter[i]);
							copyStandardFilter.value = copyStandardFilter.label + ": " + splitValue[j];
							filterTokens.push(copyStandardFilter);
						}
					} else if (advanceFilter[i].filterType !== "MULTICOMBOBOX") {
						filterTokens.push(advanceFilter[i]);
					} else { //to change
						if (advanceFilter[i].displayValue && !advanceFilter[i].displayValue.includes(",")) {
							advanceFilter[i].value = advanceFilter[i].label + ": " + advanceFilter[i].displayValue;
						}
						filterTokens.push(advanceFilter[i]);
					}
				}
		...
		getCustomAttributes: function () {
			...
			var surl = "/WorkboxJavaService/customHeaders/getCustomTemplates?userId=" + userID + "&processName=STANDARD";
			...
			var url = "/WorkboxJavaService/customHeaders/getCustomTemplates?userId=" + userID + "&processName=" + processName;
		...
		onSaveTableConfigFn: function () {
			...
			if (!validationCheck) {
				var url = "/WorkboxJavaService/customHeaders/saveCustomAttributes?userId=" + userID;
		...
		onDeleteDraftFn: function (oEvent) {
			...
			var url = "/WorkboxJavaService/tasks/deleteDraft/" + object.processId + "/" + object.requestId;
			this.doAjax(url, "GET", null, function (oData) {
				oTaskInboxModel.setProperty("/openRightClickMenu", false);
				this.setInboxPanel();
				this._showToastMessage(oData.message);
		...
		pinTaskFn: function (oEvent) {
			...
			var url = "/WorkboxJavaService/inbox/pinned";
			var status = true;
		...
		getSigningProcessURl: function (processId) {
			var hUrl = "/WorkboxJavaService/docusign/sign";
			var data = {
				"processId": processId,
		...
		viewProcessFlow: function (processId, taskId, showDetails, oEvent) {
			...
			var url = "/WorkboxJavaService/task/details";
			this.doAjax(url, "POST", requestPayload, function (oData) {
					...
					if (oData.tasks.length > 0) {
						...
						this._showToastMessage(this.getOwner().getModel("i18n").getResourceBundle().getText("TASK_NOT_FOUND_TEXT"));
		...
		addProcessFlowTasks: function (bindingPath, bindingObject) {
			...
			// show completed time for completed tasks
			var timeOfTask =
				"{path: 'oProcessFlowModel>createdAtInString' ,formatter: 'oneapp.incture.workbox.util.formatter.wbProcessFlowTime'}";
			if (bindingObject.status.toLowerCase().includes("complete") || bindingObject.status.toLowerCase().includes("done") || bindingObject
				.status.toLowerCase().includes("accept") || bindingObject.status.toLowerCase().includes("approve") || bindingObject.status.toLowerCase()
				.includes("reject") || bindingObject.status.toLowerCase().includes("decline")) {
				timeOfTask =
					"{path: 'oProcessFlowModel>completedAtInString' ,formatter: 'oneapp.incture.workbox.util.formatter.wbProcessFlowTime'}";
			}
			...
			var oTaskTray = new sap.m.VBox({
					renderType: "Bare",
					width: "100%",
					visible: "{parts: ['oProcessFlowModel>status', 'oProcessFlowModel>eventId','oProcessFlowModel>/selTaskEvent/eventId','oProcessFlowModel>/showDetails','oProcessFlowModel>/selTaskEvent/origin'] ,formatter: 'oneapp.incture.workbox.util.formatter.wbSetProcessFlowTaskBackground'}"
				}).addItem(taskHeader)
			...
			var taskCLI = new sap.m.CustomListItem({
				type: "{parts: ['oProcessFlowModel>/showDetails','oProcessFlowModel>status','oProcessFlowModel>/selTaskEvent/origin'] ,formatter: 'oneapp.incture.workbox.util.formatter.wbSetShowDetailsPS'}",
				press: function (oEvent) {
				...
				visible: "{parts: ['oProcessFlowModel>status', 'oProcessFlowModel>eventId','oProcessFlowModel>/selTaskEvent/eventId','oProcessFlowModel>/showDetails','oProcessFlowModel>/selTaskEvent/origin'] ,formatter: 'oneapp.incture.workbox.util.formatter.wbSetProcessFlowTaskArrow'}"
		...
		getJiraStatusDropdown: function (oActionHeader, taskId) {
			var url = "/WorkboxJavaService/jira/getStatus?taskId=" + taskId;
			this.doAjax(url, "GET", null, function (oData) {
				if (oData) {
		...
		jiraActionUpdateFn: function (oActionPayload) {
			...
			var aurl = "/WorkboxJavaService/inbox/actions";
		...
		jabilTasksUpdateFn: function (oActionPayload) {
			...
			if (processName === "jabilinventorymanagement" || processName === "inventoryparentworkflow") {
				...
				var zurl = "/WorkboxJavaService/workFlow/updateContextDataLineItems?processId=" + processId;
			...
			if (processName !== "ic_manager_approval_process") {
				var url = "/WorkboxJavaService/workFlow/updateContextData?processId=" + processId + "&processName=" + processName;
		...
		fnGetChatUsers: function () {
			...
			this.doAjax(url, "GET", null, function (oData) {
				...
				var aFilter = aWbUserDetails.filter(function (obj) {
					return (obj.id !== sUserId);
				});
				oTaskLevelChat.setProperty("/aUserDetails", aFilter);
		...
		fnTaskUploadAttachment: function (sChatId, message, sSendToUserId, sUserId, aTagged, aMemberList) {
			...
			$.each(aAttachment, function (index, oFile) {
				...
				oFormData.append("fileName", sFileName + "." + sFileExtension);
				oFormData.append("fileType", sFileType);
				aAttachmentDetails.push({
					"fileName": sFileName + "." + sFileExtension,
					"fileType": sFileType,
		...
		fnTaskSendMessage: function (sChatId, message, aMemberPid, sUserId, sCommentID, aTagged, aMemberDetails) {
			...
			this.doAjax(url, "POST", requestPayload, function (oData) {
					if (oData.data === "You are no longer a participant of this chat!") {
						...
						oTaskLevelChat.setProperty("/newMessage", "");
						aChat[aChat.length - 1].commentID = oData.commentID ? oData.commentID : "";
		...
		onSelectingSuggestionItem: function (oEvent) {
			...
			if (aFilter.length === 0) {
				aTaggedId.push({
					"tagged": sId,
					"name": oSelectedObject.displayName
				});
		...
		onLiveSearch: function () {
			...
			if (sQuery) {
				...
				var aFilter = aUserDetails.filter(function (obj) {
					return (obj.displayName.toLowerCase().includes(sQuery) && obj.id !== sUserId);
				});
				// var sUserId = this.oAppModel.getProperty("/loggedInUserDetails/userId");
		...
		wbListSelect: function () {
			...
			this.oAppModel.refresh(true);
			workbox.wbCommonSubheaderActionNew(that, status, taskType, tab, currentView, currentViewPage, multiselect, reset, null,
				hideButtons);
		...
		lineItemActionFn: function (oEvent) {
			...
			if (selectedAction === "Claim" || selectedAction === "Release") {
				...
				isPurchaseOrder = false; //added to let sign in pop up
				if (!isPurchaseOrder) {
		...
		checkSignInData: function () {
			...
			if (oAppModel.getProperty("/loggedInPassword")) {
				...
				this.doAjax("/WorkboxJavaService/idpMapping/validateUser", "POST", data, function (oData) {
		...
		actionServiceCall: function (oActionPayload) {
			...
			if (((processName !== "analystprocessworkflow" || processName !== "jabilinventorymanagement" || processName !==
				...
				if (oActionPayload.task.length === 0) {
					...
					if (validation) {
						var that = this;
						var url = "/WorkboxJavaService/inbox/actions";
		...
		bulkDeleteDraft: function () {
			...
			for (var i = 0; i < requestIds.length; i++) {
				var url = "/WorkboxJavaService/tasks/deleteDraft/" + requestIds[i].processId + "/" + requestIds[i].requestId;
				this.doAjax(url, "GET", null, function (oData) {
					this._showToastMessage(oData.message);
					that.onClickFilterDetail();
					this.oAppModel.setProperty("/enableBulkDeleteButton", false);
