sap.ui.define([
	...
	return BaseController.extend("oneapp.incture.workbox.controller.ProcessFlow", {
		...
		showEventDetails: function (eventDetails) {
			...
			var url = "/WorkboxJavaService/detailPage/dynamicDetails?taskId=" + eventDetails.eventId +
				"&$select=taskStory";
			...
			this.doAjax(url, "GET", null, function (oData) {
				...
				for (var j = 0; j < oData.contentDto.length; j++) {
					...
					if (oData.contentDto[j].dataType === "LINEITEM") {
						var subdata = oData.contentDto[j].contentDto;
						for (var m = 0; m < subdata.length; m++) {
							var attributeValues = subdata[m].attributeValues;
					...
					if (oData.contentDto[j].dataType === "TASKDETAILS") {
						...
						if (iTaskDetails !== -1 && oData.contentDto[iTaskDetails].contentDto && oData.contentDto[iTaskDetails].contentDto.length > 0) {
							...
							if (oProcessFlow.taskDetails.process.name === "Courseregistration" || oProcessFlow.taskDetails.process.name ===
								"ReimbursementofregisteredcoursesDemo" || oProcessFlow.taskDetails.process.name === "CourseRegistration" || oProcessFlow.taskDetails
								.process.name ===
								"Reimbursementofregisteredcourses" || oProcessFlow.taskDetails.process.name === "CourseRegistrationDemo") {
								var contDto = oData.contentDto;
								var subdata = contDto[j].contentDto;
								...
								contDto[j].contentDto = subdata;
								if (itotalCost) {
									oData.contentDto[itotalCost[0]].contentDto[itotalCost[1]].value = tempTotal;
								}
							}

							oProcessFlow.customAttributesDto = oProcessFlow.customAttributesDto.concat(oData.contentDto[iTaskDetails].contentDto);
						}
					}
							...
							for (var i = 0; i < attachmentDto.length; i++) {
								if (attachmentDto[i].attributeValues.length > 0) {
									...
									if (checkIndex) {
										...
										for (var p = 0; p < attributeData.length; p++) {
											if (attributeData[p].key === "attachmentName") {
												attachmentDto[i].attachmentName = attachmentDto[i].attributeValues[p].attributeValue;
											} else if (attributeData[p].key === "attachmentSize") {
												attachmentDto[i].attachmentSize = attachmentDto[i].attributeValues[p].attributeValue;
											} else if (attributeData[p].key === "attachmentType") {
												attachmentDto[i].attachmentType = attachmentDto[i].attributeValues[p].attributeValue;

											} else if (attributeData[p].key === "attachmentId") {
												attachmentDto[i].attachmentId = attachmentDto[i].attributeValues[p].attributeValue;
											}
										}
										checkIndex = false;
									}
