sap.ui.define([
	...
	return BaseController.extend("oneapp.incture.workbox.controller.TaskDetail", {
		...
		getTaskDetails: function () {
			...
			if (object.isCustomLayout || processName === "analyst_appproval_process" || processName === "inventoryparentworkflow" ||
				processName ===
				...
				if (processName === "analyst_appproval_process" || processName === "inventoryparentworkflow" || processName ===
					...
					url = "/WorkboxJavaService/detailPage/dynamicDetailsJabil?taskId=" + taskId;
					oTaskDetailModel.setProperty("/roleEditableValue", true);
				} else {
					url = "/WorkboxJavaService/detailPage/dynamicDetailFromContext?taskId=" + taskId;
				}
				...
				url = "/WorkboxJavaService/detailPage/dynamicDetails?taskId=" + taskId;
				this.doAjax(url, "GET", null, function (oData) {
					oTaskDetailModel.setProperty("/contentDto", oData.contentDto);
					// EA POC 
					if (processName === "CourseRegistrationDemo" || processName === "ReimbursementofregisteredcoursesDemo") {
						var contDto = oData.contentDto;
						var tempTotal = 0,
							itotalCost;
						for (var i = 0; i < contDto.length; i++) {
							if (contDto[i].dataType === "LINEITEM") {
								var subdata = contDto[i].contentDto;
								for (var j = 0; j < subdata.length; j++) {
									var attributeValues = subdata[j].attributeValues;
									for (var k = 0; k < attributeValues.length; k++) {
										if (attributeValues[k].key.toLocaleLowerCase() === "tuition fees") {
											tempTotal = tempTotal + parseInt(attributeValues[k].attributeValue, 10);
										}
									}
								}
							} else if (contDto[i].dataType === "TASKDETAILS") {
								var subdata = contDto[i].contentDto;
								for (var z = 0; z < subdata.length; z++) {
									if (subdata[z].label.toLocaleLowerCase() === "total cost") {
										tempTotal = tempTotal + parseInt(subdata[z].value, 10);
										itotalCost = "/contentDto/" + i + "/contentDto/" + z + "/value";
										break;
									}
								}
								contDto[i].contentDto = subdata;
							}
						}
						if (itotalCost) {
							oTaskDetailModel.setProperty(itotalCost, tempTotal);
						}
					}
		...
		onQuantityUpdate: function (oEvent) {
			...
			var url = "/WorkboxJavaService/ECC/updatePRDetails";
		...
		addCellItem: function (bindingObject, oTaskDetailModel) {
			var item;
			if (bindingObject.keyType === "INPUT" || bindingObject.keyType === "INPUT NUMERIC") {
			...
			} else if (bindingObject.keyType === "DATETYPE") {
			...
			} else if (bindingObject.keyType === "SWITCH") {
				item = new sap.m.Switch({
					state: "{oTaskDetailModel>keyValue}",
						...
						if (oDynamicGridDto[i].contentDto.length !== 0) { // if contentDto.length === 0 then will not render lineItem and Attachments
							...
							if (flag) {
								...
								this.createDynamicControls(newContent, oDynamicGridDto[i].contentDto, "oDynamicGridDto/" + i + "/contentDto",
									"oTaskDetailModel", oDynamicGridDto[i].title);
		...
		createDynamicControls: function (oGrid, dynamicControlData, sDynamicDto, sModelName, titleText) {
			...
			for (var i = 0; i < dynamicControlData.length; i++) {
				...
				if (this.oAppModel.getProperty("/taskObjectDetails/processName") === "Reimbursementofregisteredcourses" || this.oAppModel.getProperty(
						"/taskObjectDetails/processName") === "CourseRegistration" || this.oAppModel.getProperty("/taskObjectDetails/processName") ===
					"ReimbursementofregisteredcoursesDemo" || this.oAppModel.getProperty(
						"/taskObjectDetails/processName") === "CourseRegistrationDemo") {
					if (dynamicControlData[i].dataType !== "LINEITEM") {
						oGrid.setDefaultSpan("L4 M6 S12");
					}
					if (dynamicControlData[i].label === "Are you being reimbursed by any organisation or source") {
						this.oAppModel.setProperty("/taskObjectDetails/reasonTrue", dynamicControlData[i].value);
					}
					if (dynamicControlData[i].label === "If Yes what organization") {
						visibleValue = false;
						if (this.oAppModel.getProperty("/taskObjectDetails/reasonTrue") === "Yes") {
							visibleValue = true;
						}
					}
				}
				if (this.oAppModel.getProperty("/taskObjectDetails/processName") === "ProjectProposalDocumentApproval") {
					...
					if (dynamicControlData[i].key === "gia69b268cfe" || dynamicControlData[i].label.includes("Project Start Date")) {
						editableValue = false;
							...
							dynamicControlData[i].dataType === "INPUT" || dynamicControlData[i].dataType === "INPUT NUMERIC" || dynamicControlData[i].dataType ===
				...
				if (visibleValue) {
					...
					if ((dynamicControlData[i].dataType).toLowerCase() === "select") {
					...
					} else if ((dynamicControlData[i].dataType).toLowerCase() === "input numeric" || (dynamicControlData[i].dataType).toLowerCase() ===
						"input numeric-calculate") {
						...
						if (runTime) {
							...
							if (dynamicControlData[i].runTimeType === "group") {
								this.doAjax("/WorkboxJavaService" + dynamicControlData[i].url, "GET", null, function (oData) {
							...
							var dropdownUrl = "/WorkboxJavaService/crossConstant/getConstants/" + dynamicControlData[i].key;
						...
						if (i === 0) {
							newControl = this.createLineItemTable(dynamicControlData, titleText);
		...
		createLineItemTable: function (dynamicControlData, tableTitle) {
			var that = this;
			var oTaskDetailModel = this.getView().getModel("oTaskDetailModel");
			...
			// var oTitle;
			// if(tableTitle){oTitle = tableTitle;}else{oTitle = this.getView().getModel("i18n").getResourceBundle().getText("LINE_ITEM_TEXT");}
		...
		generateFormData: function (oEvent, oTaskDetailModel) {
			...
			if (check) {
				...
				//var url = "/WorkboxJavaService/inventory/getFormDetails?taskId=" + taskId + "&index=" + index;
				var url = "/WorkboxJavaService/inventory/getFormDetails?taskId=" + taskId;
				this.doAjax(url, "GET", null, function (oData) {
					if (!oData.length) {
						form = oData.formData;
					} else {
		...
		saveAttributesFn: function () {
			...
			if (contentDto) {
				for (var j = 0; j < contentDto.length; j++) {
					...
					if (typeof (contentDto[j].value) === "number") {
						contentDto[j].value = contentDto[j].value.toString();
					}
			...
			if (customAttributes.length) {
				var url = "/WorkboxJavaService/tasks/updateTaskAttributes";
		...
		updateAttributesFn: function () {
			...
			var processId = this.oAppModel.getProperty("/taskObjectDetails/processId");
			var url = "/WorkboxJavaService/workFlow/updateWorkflowContextData?processId=" + processId;
			this.doAjax(url, "POST", postData, function (oData) {
					this._showToastMessage(oData.message);
					this.getTaskDetails();
				}.bind(this),
		...
		reCreateTask: function (template) {
			...
			if (!missingField) {
				this.doAjax("/WorkboxJavaService/tasks/createTasks", "POST", template, function (oData) {
		...
		closePPDAAction: function () {
			...
			this.doAjax("/WorkboxJavaService/tasks/updateTaskAttributes", "POST", oPayload, function (oData) {
				if (oData.status !== "FAILURE") {
					this._showToastMessage(this.oAppModel.getProperty("/taskObjectDetails/taskDescription") + " " + this.getView().getModel("i18n")
						.getResourceBundle().getText("CLOSED_SUSSESSFULLY_TEXT") + ".");
					setTimeout(function () {
						oTaskDetailModel.setProperty("/isUpdated", false);
						oTaskDetailModel.setProperty("/isClosePPDA", false);
						oTaskDetailModel.setProperty("/isResubmitPPDA", false);
		...
		closeTaskDetailPage: function () {
			...
			this.oAppModel.setProperty("/loadTableUserManagement", false);
		...
		wbDataForHeaderButtons: function () {
			...
			/********** task owner-Chnages by karishma **********/
			if (bIsTaskOwner) {
				workbox.wbCommonSubheaderActionNew(that, status, taskType, tab, currentView, currentViewPage);
			} else {
