sap.ui.define([
	...
	return BaseController.extend("oneapp.incture.workbox.controller.Chat", {
		...
		onInit: function () {
			...
			oRouter.attachRoutePatternMatched(function (oEvent) {
				...
				if (oEvent.getParameter("name") === "Chat") {
					...
					} else if (oCollaborationModel.getProperty("/bFromNotification")) {
						var oChatNotification = oCollaborationModel.getProperty("/oChatNotification");
						var sChatType = oChatNotification.eventName.split(" ")[1];
						var sChatList = "directMessage";
						if (sChatType === "personal") {
							sChatList = "directMessage";
						} else if (sChatType === "context") {
							sChatList = "channelsTasks";
						} else if (sChatType === "group") {
							sChatList = "groups";
						}
						this.oAppModel.getProperty("/sideNavItemProperties").setSelectedKey(sChatList);
						this.selectedChatScreen(sChatList);
					} else {
						this.getChatUserList("directMessage");
					}
					/**********Collaboration Changes End -BY Karishma************/
		...
		selectedChatScreen: function (selectedKey) {
			...
			oCollaborationModel.setProperty("/searchValue", "");
			...
			this.doAjax(url, "GET", null, function (oData) {
					...
					if (aChatList) {
						if (sChatType === "personal") {
							...
							if (oCollaborationModel.getProperty("/bFromNotification")) {
								for (var i = 0; i < aChatList.length; i++) {
									for (var j = 0; j < aChatList[i].tasks.length; j++) {
										if (aChatList[i].tasks[j].chatID === oChatNotification.id) {
											var sNotificationChatIndex = "/aChatList/" + i + "/tasks/" + j;
											var iProcessId = i;
											break;
										}
									};
								}
								that.oAppModel.setProperty("/selectedChatPathNav", sNotificationChatIndex);
								aChatList[iProcessId].bExpandPanel = true;
							} else {
								aChatList[0].bExpandPanel = true;
							}
						}
						if (sChatType === "context") {
							...
							if (this.oAppModel.getProperty("/toChat") || oCollaborationModel.getProperty("/bFromNotification")) {
								...
								oCollaborationModel.setProperty("/bFromNotification", false);
							} else {
								that.oAppModel.setProperty("/selectedChatPathNav", oFirstItem.getBindingContext("oCollaborationModel").sPath);
							...
							if (oCollaborationModel.getProperty("/bFromNotification")) {
								for (var i = 0; i < aChatList.length; i++) {
									if (aChatList[i].chatID === oChatNotification.id) {
										var sNotificationChatIndex = i;
										break;
									}
								}
								var sDisplayName = (selectedKey === "directMessage") ? aChatList[sNotificationChatIndex].senderDetails[0].displayName :
									aChatList[sNotificationChatIndex].chatName;
								oCollaborationModel.setProperty("/selectedUserName", sDisplayName);
								oCollaborationModel.setProperty("/bFromNotification", false);
							} else {
								var sNotificationChatIndex = 0;
							}
							that.fnCheckIsActive(aChatList[sNotificationChatIndex].isActive);
							oCollaborationModel.setProperty("/selectedChatId", aChatList[sNotificationChatIndex].chatID);
							oCollaborationModel.setProperty("/sSelectedChatType", aChatList[sNotificationChatIndex].chatType);
							oCollaborationModel.setProperty("/aMemberDetails", aChatList[sNotificationChatIndex].memberDetails);
							oCollaborationModel.setProperty("/aMemberPid", aChatList[sNotificationChatIndex].members);
							oCollaborationModel.setProperty("/iPageCount", 0);
							oCollaborationModel.setProperty("/userList/" + sNotificationChatIndex + "/unreadMessageCount", 0);
							that.byId("idChatList").setSelectedItem(that.byId("idChatList").getItems()[sNotificationChatIndex]);
							that.getChatHistory(sUserId, aChatList[sNotificationChatIndex].chatID);
							oCollaborationModel.refresh(true);
			...
			$.ajax({
				...
				success: function (data, textStatus, XMLHttpRequest) {
					...
					if (sCommentID) {
						...
						that._showToastMessage(that.getResourceBundle().getText("UPLOAD_FAILED_TEXT"));
					}
				},
			...
			this.doAjax(url, "POST", oPayload, function (oData) {
					...
					if (!bShowSideContent) {
						if (sChatType === "personal") {
							...
							oCollaborationModel.setProperty("/selectedChatId", oData.data);
		...
		onLiveSearch: function () {
			...
			if (sQuery) {
				...
				// } else {
				aSearch = aSearch ? aSearch : [];
				var aFilter = aSearch.filter(function (obj) {
					return (obj.searchName.toLowerCase().includes(sQuery) && obj.id !== sUserId);
				});
				var sMessage = "";
				if (aFilter.length === 0) {
					if (selectedScreen === "directMessage") {
						sMessage = this.getResourceBundle().getText("NO_USER_FOUND_TEXT");
					} else if (selectedScreen === "channelsTasks") {
						sMessage = this.getResourceBundle().getText("NO_TASK_FOUND_TEXT");
					} else if (selectedScreen === "groups") {
						sMessage = this.getResourceBundle().getText("NO_GROUP_FOUND_TEXT");
					} else if (selectedScreen === "pinned") {
						sMessage = this.getResourceBundle().getText("NO_PINCHAT_FOUND_TEXT");
					} else if (selectedScreen === "favorites") {
						sMessage = this.getResourceBundle().getText("NO_FAVORITE_MESSAGE_TEXT");
					}
					this._showToastMessage(sMessage);
				}
		...
		showSideContent: function () {
			...
			if (bShowSideContent) {
				...
				oCollaborationModel.setProperty("/sidePanelSearchValue", "");
			}
		},
		...
		onLiveSearchSidePanel: function () {
			...
			if (sQuery) {
				...
				if (aFilter.length === 0) {
					this._showToastMessage(this.getResourceBundle().getText("NO_USER_FOUND_TEXT"));
				}
				// var sUserId = this.oAppModel.getProperty("/loggedInUserDetails/userId");
				// var url = "/ActJavaService/chat/search/" + sUserId + "/" + sQuery;
				// this.doAjax(url, "GET", null, function (oData) {
		...
		onSelectNameSidePanel: function (oEvent) {
			...
			if (oEvent.getParameter("query")) {
				...
				if (!(aFilter.length > 0)) {
					...
					this._showToastMessage(this.getResourceBundle().getText("ALREADY_MEMBER_TEXT"));
				}
				oEvent.getSource().clear();
		...
		onCreateGroupPress: function () {
			...
			if (sGroupName) {
				var sUserId = this.oAppModel.getProperty("/loggedInUserDetails/userId");
				var aAllUsers = this.getModel("oConstantsModel").getProperty("/allUsers");
				var aOriginalUserList = $.extend(true, [], aAllUsers);
				oCollaborationModel.setProperty("/aOriginalUserList", aOriginalUserList);
				var aFilter = aAllUsers.filter(function (obj) {
					return (obj.userId !== sUserId);
				});
				this.getModel("oConstantsModel").setProperty("/allUsers", aFilter);
		...
		onCloseResourceFragment: function () {
			...
			var aOriginalUserList = oCollaborationModel.getProperty("/aOriginalUserList");
			this.getModel("oConstantsModel").setProperty("/allUsers", aOriginalUserList);
		},
