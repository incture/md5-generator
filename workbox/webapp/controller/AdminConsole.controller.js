sap.ui.define([
	...
	return BaseController.extend("oneapp.incture.workbox.controller.AdminConsole", {
		...
		onInit: function () {
			...
			otaskCreationModel.setProperty("/uiGeneration");
			var oDropDownValueseModel = new sap.ui.model.json.JSONModel();
			oDropDownValueseModel.setProperty("/dropDownValues", {});
			this.getView().setModel(oDropDownValueseModel, "oDropDownValueseModel");
		...
		onGraphListItemSelect: function (oEvent, isClone) {
			var view = this;
			var that = this.getModel("oAdvanceFilterModel").getProperty("/appController");
				...
				.addItem(new sap.m.HBox({
					...
					.addItem(new sap.m.Button({
						...
						press: function (oEvt) {
							...
							if (bindingObject.layoutAttributesData.length) {
		...
		createLayoutFn: function () {
			var oDefaultDataModel = this.getModel("oDefaultDataModel");
			var layoutData = {
				"layoutId": Math.random().toString(20).substr(2, 24),
				"layoutName": "",
				"label": "",
				"layoutSize": "",
				"parentLayoutName": "",
		...
		editAttributeSubfn: function () {
			...
			oDefaultDataModel.setProperty("/editAttributeObject/layoutAttributesData/" + index, oDefaultDataModel.getProperty(
				"/editObjectDetails"));
			oDefaultDataModel.setProperty("/isLayoutChanged", true);
			oDefaultDataModel.refresh(true);
		...
		changeProperty: function (oEvent, property) {
			var oDefaultDataModel = this.getModel("oDefaultDataModel");
			if (oDefaultDataModel.getProperty("/setEnabled") === false && this.oAppModel.getProperty("/currentViewPage") ===
				'userRoleManagement') {
		...
		changeDataTypeProperty: function (oEvent) {
			...
			if (dataType === "INPUT NUMERIC") {
				var processLevelAttributes = oDefaultDataModel.getProperty("/processLevelAttributes");
				var filteredAttributes = processLevelAttributes.filter(function (item) {
					if (item.dataType === "INPUT NUMERIC")
						return item;
				});
				filteredAttributes.push(object);
				...
				processLevelOperatorAttributes = filteredAttributes.concat(operators);
		...
		//set none to destination if the task type is rule based.
		changeTaskTypeProperty: function (oEvent) {
			this.oAppModel.setProperty("/isChanged", true);
			var oDefaultDataModel = this.getModel("oDefaultDataModel");
			if (oEvent.getSource().getSelectedKey() === "Rule Based") {
				oEvent.getSource().getBindingContext("oDefaultDataModel").getObject().targetId = [""];
			}
			if (oDefaultDataModel.getProperty("/setEnabled") === false) {
				var object = oEvent.getSource().getBindingContext("oDefaultDataModel").getObject();
				if (object.isEdited !== 2) {
		...
		createWorkflowTaskValidation: function () {
			var oDefaultDataModel = this.getModel("oDefaultDataModel");
			var that = this;
			var calculativeCheck = 2;
			taskManagement.createWorkFlowTaskValidationTM(that, oDefaultDataModel, calculativeCheck);
		},
		...
		changeSourceDestTaskFn: function (oEvent, param) {
			var oDefaultDataModel = this.getModel("oDefaultDataModel");
			var object = oEvent.getSource().getBindingContext("oDefaultDataModel").getObject();
			object[param] = oEvent.getSource().getSelectedKeys();
			if (oDefaultDataModel.getProperty("/setEnabled") === false) {
				if (object.isEdited !== 2) {
					object.isEdited = 1;
				}
			}
			if (object[param].length > 1 && object[param].includes("")) {
				object[param].splice(object[param].indexOf(""), 1);
			} else if (object[param].length === 0) {
				object[param] = [""];
			}
			oEvent.getSource().setSelectedKeys(object[param]);
		},
		...
		openOwnerSelectionFn: function (oEvent) {
			...
			if (teamDetailDto) {
				...
				if (teamDetailDto[currentIndex].ownerSelectionRules.length === 0 || teamDetailDto[currentIndex].group.length > 0 ||
					teamDetailDto[
						currentIndex].individual.length > 0) {
					...
					var rules = teamDetailDto[currentIndex].ownerSelectionRules;
					var count = 0;
					for (var k = 0; k < rules.length; k++) {
						if (rules[k].isDeleted === false) {
							count = 1;
							break;
						}
					}
		...
		openSetRuleTaskFn: function (oEvent) {
			...
			oEvent.getSource().getBindingContext("oDefaultDataModel").getObject().ownerSequenceType = {
				"ownerSequType": "Group",
				"attrTypeId": "",
				"attrTypeName": "",
				"orderBy": "ASC"
		...
		workFlowPreviewFn: function () {
			...
			var oDefaultDataModel = this.getModel("oDefaultDataModel");
			oDefaultDataModel.setProperty("/taskTemplateData", []);
			index = oDefaultDataModel.getProperty("/createWorkFlowData/teamDetailDto").length - 1;
			oDefaultDataModel.setProperty("/selectedTaskTemplate", index);
			this.workFlowPreviewSetDataFn(index);
			this.setPreviewProperties();
			var taskObj = oDefaultDataModel.getProperty("/createWorkFlowData/teamDetailDto/" + index);
			this.templateServiceCallWF(taskObj.processName, taskObj.eventName);
		},

		//setting properties for graph in Preview
		setPreviewProperties: function () {
			...
			oDefaultDataModel.refresh(true);
			//new change
			oDefaultDataModel.setProperty("/isLayoutFraming", false);
		...
		showTaskDetailsPreviewFn: function (oEvent) {
			...
			var templateId = oEvent.getSource().getBindingContext("oDefaultDataModel").getObject().templateId;
			if (templateId !== "startTask" && templateId !== "endTask") {
				oEvent.getSource().getParent().getParent().addStyleClass("wbAdminPreviewGraphContainer");
				var taskDto = oDefaultDataModel.getProperty("/previewTaskDto");
				var taskObj = oEvent.getSource().getBindingContext("oDefaultDataModel").getObject();
				this.templateServiceCallWF(taskObj.processName, taskObj.eventName);
				this.fetchTaskAttributesFnWF(taskObj.customAttributes);
				oDefaultDataModel.setProperty("/selectedTask", taskObj);
				for (var i = 0; i < taskDto.length; i++) {
				...
				oEvent.getSource().getParent().getParent().removeStyleClass("wbAdminPreviewGraphContainer");
			}
		},

		//getting layout details
		templateServiceCallWF: function (processName, taskName) {
			var that = this;
			var oDefaultDataModel = this.getModel("oDefaultDataModel");
			var tURl = "/WorkboxJavaService/detailPage/taskConfiguration?processName=" + processName + "&taskName=" + taskName;
			oDefaultDataModel.setProperty("/currentTemplateTab", taskName);
			...
			this.doAjax(tURl, "GET", null, function (data) {
				oDefaultDataModel.setProperty("/isChanged", false);
				this.oAppModel.setProperty("/isChanged", false);
				oDefaultDataModel.setProperty("/isLayoutChanged", false);
				if (data.taskTemplateData.length) {
					oDefaultDataModel.setProperty("/taskTemplateData", data.taskTemplateData[0]);
					that.setLayoutFnWF();
				} else {
		...
		///new change
		//service call to fetch all the task attributes
		fetchTaskAttributesFnWF: function (existingAttr) {
			var oDefaultDataModel = this.getModel("oDefaultDataModel");
			var customData = [],
				attrObj;
			for (var j = 0; j < existingAttr.length; j++) {
				attrObj = {
					"layoutId": "",
					"key": existingAttr[j].key,
					"keyLabel": existingAttr[j].label,
					"keyType": existingAttr[j].dataType,
					"isEditable": existingAttr[j].isEditable,
					"sourceIndex": "",
					"isMandatory": existingAttr[j].isMandatory,
					"isVisible": existingAttr[j].isVisible,
					"hasAction": false,
					"actionURL": "",
					"sequence": j + 1,
					"valueHelpId": "",
					...
					"index": null,
					"validForUsage": null,
					"runTimeType": existingAttr[j].runTimeType,
					"isRunTime": existingAttr[j].isRunTime
				};
				customData.push(attrObj);
			}
			oDefaultDataModel.setProperty("/layoutCustomData", customData);
			oDefaultDataModel.refresh();
		},

		//to show layout template
		showLayoutFraming: function () {
			var oDefaultDataModel = this.getModel("oDefaultDataModel");
			oDefaultDataModel.setProperty("/isLayoutFraming", true);
		},
		...
		//to delete all the custom layouts of a Task
		deleteAllLayoutWF: function (oEvent) {
			var oDefaultDataModel = this.getModel("oDefaultDataModel");
			oDefaultDataModel.setProperty("/taskTemplateData/layoutsData", []);
			this.updateTaskConfigFnWF();
		},

		//to create the layout structure
		setLayoutFnWF: function () {
			var that = this;
			var oDefaultDataModel = this.getModel("oDefaultDataModel");
			oDefaultDataModel.setProperty("/busyIndicators/templateCreation", true);
			//new change
			// var selectedTaskIndex = oDefaultDataModel.getProperty("/selectedTaskTemplate");
			this.getView().byId("ID_CREATE_TASK_DETAIL_LAYOUT_WF").removeAllItems();
			//new change
			var i18n = that.getView().getModel("i18n").getResourceBundle();
			var headerDto = [{
				"label": i18n.getText("SEQUENCE_TEXT")
			}, {
				"label": i18n.getText("LABEL_TEXT")
			}, {
				"label": i18n.getText("KEY_TEXT")
			...
			var vBox = new sap.m.VBox({
				...
				var bindingObject = context.getObject();
				var VBox = that.createControlsFnWF(bindingObject, oDefaultDataModel);
				var subLayout = new sap.m.VBox({
					renderType: "Bare",
					alignItems: "Center",
					width: "98.5%"
					...
					var subObject = subContext.getObject();
					subObject.level = "2";
					var subVBox = that.createControlsFnWF(subObject, oDefaultDataModel);
					var sSubVBox = new sap.m.VBox({
						renderType: "Bare",
						alignItems: "Center",
						width: "98.5%"
						...
						var sSubObj = sSubContext.getObject();
						sSubObj.level = "3";
						var container = that.createControlsFnWF(sSubObj, oDefaultDataModel);
						return container;
					}.bind(that));
					subVBox.addItem(sSubVBox);
					return subVBox;
			...
			this.getView().byId("ID_CREATE_TASK_DETAIL_LAYOUT_WF").addItem(vBox);
			oDefaultDataModel.setProperty("/busyIndicators/templateCreation", false);
		},

		//adding controls to the layout
		createControlsFnWF: function (bindingObject, oDefaultDataModel) {
			var that = this;
			var i18n = that.getView().getModel("i18n").getResourceBundle();
			var subLayoutBtn = false;
			var attributeVisibleBtn = true;
			var text;
			var InitialGrid = true;

			if (bindingObject.layoutType.toLowerCase() === "icontabbar" ||
				bindingObject
				.layoutType
				.toLowerCase() === "forms") {
				text = i18n.getText("ADD_TEXT") + " " + i18n.getText("ATTRIBUTE_TEXT");
				...
				attributeVisibleBtn = false;
				InitialGrid = false;
			} else if (bindingObject.layoutType.toLowerCase() === "grid") {
				text = i18n.getText("ADD_TEXT") + " " + i18n.getText("ATTRIBUTE_TEXT");
			} else if (bindingObject.layoutType.toLowerCase() === "table" || bindingObject.layoutType.toLowerCase() === "actions") {
				text = i18n.getText("ADD_TEXT") + " " + i18n.getText("COLUMN_TEXT");
				if (bindingObject.layoutType.toLowerCase() === "actions") {
			...
			var titleContent = new sap.m.HBox({
				...
				.addItem(new sap.m.HBox({
						...
						visible: subLayoutBtn
					}).addStyleClass("wbDefaultButtonClass sapUiTinyMarginEnd"))
					.addItem(new sap.m.Button({
						text: text,
						press: function (oEvt) {
							that.showAttributesLayoutFnWF(bindingObject);
						},
						visible: attributeVisibleBtn
					}).addStyleClass("wbDefaultButtonClass sapUiTinyMarginEnd"))
					.addItem(new sap.m.Button({
						text: i18n.getText("REMOVE_TEXT"),
						icon: "sap-icon://decline",
						type: "Reject",
						press: function (oEvt) {
							that.deleteLayoutFnWF(bindingObject, bindingObject.level);
						}
					}).addStyleClass("wbAdminMGroupsRemoveBulkBtn sapUiTinyMarginEnd")));
			...
			var dragInfo = new sap.ui.core.dnd.DragInfo({
				sourceAggregation: "items"
			});
			var dropInfo = new sap.f.dnd.GridDropInfo({
				targetAggregation: "items",
				dropPosition: "Between",
				dropLayout: "Horizontal",
				drop: that.onLayoutAttributeDrop
			});
			table.addDragDropConfig(dragInfo);
			table.addDragDropConfig(dropInfo);
			table.bindAggregation("columns", "oDefaultDataModel>/tableHeaderDto", function (
				index, context) {
				var column;
				var tableObject = context.getObject();
			...
			table.bindAggregation("items", "oDefaultDataModel>layoutAttributesData", function (index, context) {
				...
				row.addCell(
					new sap.m.HBox().addItem(
						new sap.m.Image({
							src: "images/DragIcon.png",
							height: "0.8rem"
						})).addItem(
						new sap.m.Text({
							text: rowData.sequence
				...
				row.addCell(new sap.m.CheckBox({
					...
					select: function (oEvt) {
						...
						var editable = false;
						var mandatory = false;
						that.setRowDataWF(oEvt, rowData, visible, editable, mandatory);
					}
				}));
				row.addCell(new sap.m.CheckBox({
					selected: rowData.isEditable,
				...
				row.addCell(new sap.m.Button({
					...
					tooltip: i18n.getText("EDIT_TEXT") + " " + i18n.getText("ATTRIBUTE_TEXT"),
					press: function (oEvt) {
						that.openEditAttributeFragFnWF(rowData, bindingObject, index);
					}
				})).addStyleClass("wbCustomButtonClass");
				return row;
			}.bind(that));
		...
		// sequencing the attribute on drag and drop
		onLayoutAttributeDrop: function (oInfo) {
			var oDragged = oInfo.getParameter("draggedControl"),
				oDropped = oInfo.getParameter("droppedControl"),
				sInsertPosition = oInfo.getParameter("dropPosition"),
				oGrid = oDragged.getParent(),
				oDefaultDataModel = this.getModel("oDefaultDataModel"),
				sPath = oInfo.getSource().getBindingContext("oDefaultDataModel").getPath(),
				oData = this.getModel("oDefaultDataModel").getProperty(sPath + "/layoutAttributesData"),
			...
			oDefaultDataModel.setProperty(sPath + "/layoutAttributesData", oData);
			oDefaultDataModel.setProperty("/isLayoutChanged", true);
		},

		//function to open the fragment to add layout
		createLayoutFnWF: function () {
			var oDefaultDataModel = this.getModel("oDefaultDataModel");
			var layoutData = {
				"layoutId": "",
				"layoutName": "",
		...
		//opening the custom attributes fragment to add in the layout
		showAttributesLayoutFnWF: function (layoutData) {
			var oDefaultDataModel = this.getModel("oDefaultDataModel");
			oDefaultDataModel.setProperty("/selectedLayout", layoutData);
			oDefaultDataModel.refresh(true);
		...
		//deletion of layout
		deleteLayoutFnWF: function (bindingObject, level) {
			var oDefaultDataModel = this.getModel("oDefaultDataModel");
			// var selectedTaskIndex = oDefaultDataModel.getProperty("/selectedTaskTemplate");
			var data = oDefaultDataModel.getData().taskTemplateData.layoutsData;
			for (var i = 0; i < data.length; i++) {
		...
		//opening copy layout fragment 
		openCopyLayoutFragFnWF: function () {
			if (!this._oOpenCopyLayoutFragment) {
				this._oOpenCopyLayoutFragment = this._createFragment("oneapp.incture.workbox.fragment.CopyLayoutAC", this);
				this.getView().addDependent(this._oOpenCopyLayoutFragment);
			}
		...
		//close copy layout fragment 
		closeCopyLayoutFragFnWF: function () {
			sap.ui.getCore().byId("WB_COPY_LAYOUT_LIST_ID_WF").removeSelections();
			this._oOpenCopyLayoutFragment.close();
		},

		//copy layout from the selected task 
		copyLayoutFnWF: function (oEvent) {
			var oDefaultDataModel = this.getModel("oDefaultDataModel");
			var taskName = oDefaultDataModel.getProperty(sap.ui.getCore().byId("WB_COPY_LAYOUT_LIST_ID_WF").getSelectedContextPaths()[0] +
				"/eventName");
			// var processName = oDefaultDataModel.getProperty(sap.ui.getCore().byId("WB_COPY_LAYOUT_LIST_ID_WF").getSelectedContextPaths()[0] +
			// 	"/processName");
			var template = oDefaultDataModel.getProperty("/taskTemplateData");
			for (var i = 0; i < template.length; i++) {
				if (template[i] && taskName === template[i].taskDetails.eventName) {
					oDefaultDataModel.setProperty("/taskTemplateData/layoutsData", jQuery.extend(true, [], template[i].layoutsData));
				}
			}
			oDefaultDataModel.setProperty("/isLayoutChanged", true);
			// oDefaultDataModel.setProperty("/isChanged", true);
			this.oAppModel.setProperty("/isChanged", true);
			// this.templateServiceCall(processName, taskName);
			this.closeCopyLayoutFragFnWF();
		},

		//setting the visible, editable and mandatory property of layout attributes data (if the user directly edits from table)
		setRowDataWF: function (oEvt, rowData, visible, editable, mandatory) {
			if (visible) {
				rowData.isVisible = oEvt.getParameter("selected");
			} else if (editable) {
				rowData.isEditable = oEvt.getParameter("selected");
		...
		addExtraContextFnWF: function () {
			...
			// obj.sequence = data.length + 1;
			data.push(obj);
			oDefaultDataModel.setProperty("/extraLayoutCustomData", data);
			oDefaultDataModel.refresh(true);
		},

		//add new attribute row on drop down select
		addRowOnDropDown: function (oEvent) {
			var oDefaultDataModel = this.getModel("oDefaultDataModel");
			var sPath = oEvent.getSource().getBindingContext("oDefaultDataModel").getPath();
			if (oEvent.getSource().getBindingContext("oDefaultDataModel").getObject().keyType === "DROPDOWN") {
				oDefaultDataModel.setProperty(sPath + "/dropDownValues", [{
					"key": Math.random().toString(20).substr(2, 24),
					"keyLabel": ""
				}]);
				oEvent.getSource().getParent().getParent().getParent().expandToLevel(3);
			} else {
				oDefaultDataModel.setProperty(sPath + "/dropDownValues", []);
			}
		},

		//to add extra rows for drop down values
		addNewDropDownRows: function (oEvent) {
			var oDefaultDataModel = this.getModel("oDefaultDataModel");
			var sPath = oEvent.getSource().getBindingContext("oDefaultDataModel").getPath();
			var data = oDefaultDataModel.getProperty(sPath + "/dropDownValues");
			var obj = {
				"key": Math.random().toString(20).substr(2, 24),
				"keyLabel": ""
			};
			data.push(obj);
			oDefaultDataModel.setProperty(sPath + "/dropDownValues", data);
		},

		//to select the drop down values when attribute with dropdown datatype is selected 
		selectNewAtrrToLayout: function (oEvent) {
			var oDefaultDataModel = this.getModel("oDefaultDataModel"),
				i;
			var ListItem = oEvent.getParameter("listItem");
			var data = ListItem.getBindingContext("oDefaultDataModel").getObject();
			var selectedData = oDefaultDataModel.getProperty("/selectedLayout/selectedLayoutAttributes");
			...
			if (oEvent.getParameters().selected) {
				...
				if (data.keyType === "DROPDOWN") {
					...
					selectedData = selectedData.concat(data.dropDownValues);
				} else {
					selectedData.push(data);
				}
			} else {
				for (i = 0; i < selectedData.length; i++) {
					if (selectedData[i].key === data.key) {
						selectedData.splice(i, 1);
						break;
					}
				}
				if (data.keyType === "DROPDOWN") {
					/*for (i = 0; i < selectedData.length; i++) {
						for (var j = 0; j < data.dropDownValues.length; j++) {
							if (selectedData[i].key === data.dropDownValues[j].key) {
								selectedData.splice(i, 1);
								i--;
								break;
							}
					...
					var filtered = selectedData.filter(
						function (e) {
			...
			oDefaultDataModel.setProperty("/selectedLayout/selectedLayoutAttributes", selectedData);
		},

		//adding attributes to the selected layout and to template json
		addAttributestoLayoutFnWF: function (oEvent) {
			var oDefaultDataModel = this.getModel("oDefaultDataModel");
			var selectedLayout = oDefaultDataModel.getProperty("/selectedLayout");
			var sourceKey = "";
			var sourceIndex = "";
			...
			var items = sap.ui.getCore().byId("ID_ADD_ATTRIBUTE_LAYOUT_LIST_WF").getSelectedContextPaths();
			selectedLayout.layoutAttributesData = [];
			var item;
			...
			sap.ui.getCore().byId("ID_ADD_ATTRIBUTE_LAYOUT_LIST_WF").removeSelections();

			var extraItems = sap.ui.getCore().byId("ID_ADD_EXTRA_ATTRIBUTE_LAYOUT_LIST_WF").getSelectedContextPaths();
			for (var j = 0; j < extraItems.length; j++) {
				item = oDefaultDataModel.getProperty(extraItems[j]);
				if (item.keyType) {
					item.sourceKey = sourceKey;
			...
			sap.ui.getCore().byId("ID_ADD_EXTRA_ATTRIBUTE_LAYOUT_LIST_WF").removeSelections();
			oDefaultDataModel.setProperty("/isLayoutChanged", true);
			// oDefaultDataModel.setProperty("/isChanged", true);
			this.oAppModel.setProperty("/isChanged", true);
			this.closeAddAttLayoutFragment();
		...
		//opening the edit aatribute fragment
		openEditAttributeFragFnWF: function (rowData, bindingObject, index) {
			var oDefaultDataModel = this.getModel("oDefaultDataModel");
			oDefaultDataModel.setProperty("/editAttributeObject", bindingObject);
			oDefaultDataModel.setProperty("/editAttributeIndex", index.split("-")[1]);
		...
		//updating the task template configuration post call
		updateTaskConfigFnWF: function () {
			var oDefaultDataModel = this.getModel("oDefaultDataModel");
			var postData = {};
			var templateData = oDefaultDataModel.getProperty("/taskTemplateData");
			// var processName = oDefaultDataModel.getProperty("/configSelectedProcess");
			// var taskName = this.getView().byId("WB_TASK_CONFIG_TAB_ID").getSelectedKey();
			var taskObj = oDefaultDataModel.getProperty("/selectedTask");
			...
			postData.processName = taskObj.processName;
			postData.taskName = taskObj.eventName;
			postData.origin = "Ad-hoc";
			postData.parentTaskName = "";
			postData.templateId = templateData.templateId;
			postData.layoutsData = templateData.layoutsData;
		...
		searchBudgetListFn: function (oEvent, tableId) {
			var aFilters = [];
			var sQuery = oEvent.getSource().getValue();
			if (sQuery && sQuery.length > 0) {
				var filter = new Filter([
			...
			var oBinding = this.getView().byId(tableId).getBinding("rows");
			oBinding.filter(aFilters, "Application");
		},
		/*****Workbox (Manage Individual Budget) - end*******/
		/*****Workbox (Manage Role ) - start*******/
		resetUserRoleManagement: function () {
			var oDefaultDataModel = this.getModel("oDefaultDataModel");
			oDefaultDataModel.setProperty("/isUserRoleType", "VIEW");

			var that = this.getModel("oAdvanceFilterModel").getProperty("/appController");
			oDefaultDataModel.setProperty("/enableBusyIndicators", false);
			that.getUserList();
		},
		enableRolesEditable: function () {
			var oDefaultDataModel = this.getModel("oDefaultDataModel");
			var isUserRoleType = oDefaultDataModel.getProperty("/isUserRoleType");
			if (isUserRoleType === "EDIT") {
				isUserRoleType = "VIEW";
			} else {
				isUserRoleType = "EDIT";
			}
			oDefaultDataModel.setProperty("/isUserRoleType", isUserRoleType);
		},
		applyUserRoleManagement: function (oEvent) {
			var oDefaultDataModel = this.getModel("oDefaultDataModel");
			var oPayload = {
				dto: this.getModel("oConstantsModel").getProperty("/allUsers")
			};
			oDefaultDataModel.setProperty("/enableBusyIndicators", true);
			this.doAjax("/WorkboxJavaService/idpMapping/updateUsers", "POST", oPayload, function (oData) {
				if (oData.status === "FAILURE") {
					this._showToastMessage(oData.message);
				} else {
					this._showToastMessage("User Details updated successfully");
				}
				oDefaultDataModel.setProperty("/enableBusyIndicators", false);
				this.oAppModel.setProperty("/isChanged", false);
				var that = this.getModel("oAdvanceFilterModel").getProperty("/appController");
				...
				that.getUserList();
				// oDefaultDataModel.getProperty("/isUserRoleType","VIEW")
			}.bind(this), function (oError) {}.bind(this));
		},
		searhRoleListFn: function (oEvent, tableId) {
			var aFilters = [];
			var sQuery = oEvent.getSource().getValue();
			if (sQuery && sQuery.length > 0) {
				var filter = new Filter([
