sap.ui.define([
	...
	return {
		 ...
		 */
		dynamicInstanceCreationTM: function (that, selectedItem, ExtDatePicker, bMergeSubTaskData) {
			...
			that.doAjax("/WorkboxJavaService/tasks/getProcessAttributes/" + selectedItem, "GET", null, function (oData) {
				...
				if (oData.responseMessage.status === "FAILURE") {
					...
					if (bMergeSubTaskData && oCustomTaskModel.getProperty("/customInstance")) {
						this.mergeInstancesTM(that, oData, ExtDatePicker, oCustomTaskModel);
					} else {
		...
		uploadInstancesTM: function (that, oData, ExtDatePicker, oCustomTaskModel) {
			...
			if (!mainVBox) {
				mainVBox = that.getView().byId("ID_INSTANCE_AGGEGRATION_VBOX") || sap.ui.getCore().byId("ID_INSTANCE_AGGEGRATION_VBOX") || sap.ui.core
					.UIComponent.getRouterFor(that).getView(
						'oneapp.incture.workbox.view.CreateTask').byId("ID_INSTANCE_AGGEGRATION_VBOX");
			...
			for (var j = 0; j < customData.length; j++) {
				if (customData[j].dataType === "TABLE") {
					isTable = true;
					if (oData.processName !== "FeesCalculation" && oData.processName !== "Reimbursementofregisteredcourses" && oData.processName !==
						"CourseRegistration" && oData.processName !== "ReimbursementofregisteredcoursesDemo" && oData.processName !==
						"CourseRegistrationDemo") {
						tableDto.push(customData[j]);
						customData.splice(j, 1);
					}
				}
			}
			...
			attributeData = attributeData.concat(attachmentDto);
			attributeData = attributeData.concat(tableDto);
			...
			for (var i = 0; i < attributeData.length; i++) {
				if (attributeData[i].dataType === "DROPDOWN") {
					if (!oCustomTaskModel.getProperty("/dropdownLists" + attributeData[i].key)) {
			...
			if (oData.type === "Multiple Instance") {
				...
				var mulContent = new sap.m.VBox({
					renderType: "Bare"
				}).addStyleClass("sapUiSmallMarginTop");
				mulContent.bindAggregation("items", "oCustomTaskModel>/customInstance/listOfProcesssAttributes", function (index, context) {
					var panel = new sap.m.Panel({
						expandable: isTable,
						expanded: isTable
					}).addStyleClass("wbCreateMulPanelClass");
					...
					hbox.bindAggregation("items", "oCustomTaskModel>customAttributeTemplateDto", function (headerIndex, headerPath) {
						var headerObj = headerPath.getObject();
						var headersPath = headerPath.getPath();
						var cell = contextTM.addCellItem(contextTM, headerObj, headersPath, ExtDatePicker, oCustomTaskModel, that, true);
						if (!cell) {
							cell = new sap.m.Text({});
						}
						var length = that.getModel("oCustomTaskModel").getProperty(headersPath.split("customAttributeTemplateDto/")[0] +
							"customAttributeTemplateDto").length;
						if ((length - headersPath.split("customAttributeTemplateDto/")[1]) === 1) {
							var cont = new sap.m.HBox({
									"renderType": "Bare",
									"alignItems": "Center",
									"justifyContent": "SpaceBetween"
								}).addItem(cell)
								.addItem(new sap.m.Button({
									...
									enabled: that.getModel("oCustomTaskModel").getProperty("/customInstance/listOfProcesssAttributes").length > 1 ? true : false,
									press: function (oEvent) {
										var sPath = oEvent.getSource().getBindingContext("oCustomTaskModel").getPath().split("/");
										var data = that.getModel("oCustomTaskModel").getProperty("/customInstance/listOfProcesssAttributes");
										data.splice(sPath[3], 1);
										that.getModel("oCustomTaskModel").refresh(true);
									}.bind(that)
						...
						if (headerObj.dataType === "TABLE") {
							labelVisible = false;
						}
						var VBox = new sap.m.VBox({
							renderType: "Bare",
							width: "40%",
							visible: headerObj.isVisible
						}).addItem(new sap.m.Label({
							text: headerObj.label,
							required: headerObj.isMandatory,
							visible: labelVisible
						})).addItem(cell);
						return VBox;
					}.bind(that));
					toolBar.addContent(hbox);
					panel.setHeaderToolbar(toolBar);
					if (isTable) {
						panel.bindAggregation("content", "oCustomTaskModel>" + context.getPath(), function (contentIndex, contentPath) {
							var instancePath = contentPath.getPath().split("customAttributeTemplateDto")[0] + "customAttributeTemplateDto";
							var tableContents = contextTM.addTableContentFn(contextTM, "oCustomTaskModel>" + instancePath, ExtDatePicker,
				...
				gridContent.bindAggregation("items",
					...
					function (
						...
						if (bindingObject.dataType === "TABLE") {
							VBox = contextTM.createTableInstance(contextTM, bindingObject, "oCustomTaskModel>" + bindingPath + "/tableContents",
								ExtDatePicker, oCustomTaskModel, that);
							VBox.addStyleClass("wbCreateTaskStretchControl");
						} else {
							VBox = contextTM.addGridContentSingleInstanceTM(contextTM, bindingObject, bindingPath, ExtDatePicker, oCustomTaskModel, that);
						}
						if (!VBox) {
							VBox = new sap.m.VBox({
								"renderType": "Bare"
		...
		addTableContentFn: function (contextTM, instancePath, ExtDatePicker, oCustomTaskModel, that) {
			var tableContents = new sap.m.VBox({
				"renderType": "Bare"
			});
			tableContents.bindAggregation("items", instancePath, function (index, context) {
				var table;
				var bindingObject = context.getObject();
				if (bindingObject.dataType === "TABLE") {
					var bindingPath = context.getPath();
					table = contextTM.createTableInstance(contextTM, bindingObject, bindingPath, ExtDatePicker, oCustomTaskModel, that);
					var container = new sap.m.VBox({
						...
						.addItem(new sap.m.HBox({
							...
							.addItem(new sap.m.Button({
								text: that.getView().getModel("i18n").getResourceBundle().getText("ADD_TEXT"),
								width: "5%",
								icon: "sap-icon://add",
								press: function (oEvent) {
									var obj = {};
									var object = oEvent.getSource().getBindingContext("oCustomTaskModel").getObject();
									var data = jQuery.extend(true, [], object.tableContents[0].tableAttributes);
									for (var i = 0; i < data.length; i++) {
										data[i].attachmentName = null;
										data[i].attachmentType = null;
										data[i].attachmentSize = null;
										data[i].attachmentValue = null;
										data[i].value = null;
										data[i].valueList = [];
										data[i].isEdited = 0;
									}
									obj.tableAttributes = data;
									object.tableContents.push(obj);
									that.getModel("oCustomTaskModel").refresh(true);
								}
		...
		//common method to create table
		createTableInstance: function (contextTM, bindingObject, bindingPath, ExtDatePicker, oCustomTaskModel, that) {
			var table = new sap.m.Table({}).addStyleClass("wbCreateTaskTable wbCustomTableClass");
			table.bindAggregation("columns", "oCustomTaskModel>tableContents/0/tableAttributes", function (index, context) {
				var column;
				var columnObject = context.getObject();
				column = new sap.m.Column({
					width: "auto",
					header: new sap.m.Label({
						text: columnObject.label,
						required: columnObject.isMandatory,
						wrapping: true
			...
			table.bindAggregation("items", "oCustomTaskModel>tableContents", function (index, context) {
				var row = new sap.m.ColumnListItem();
				row.bindAggregation("cells", "oCustomTaskModel>tableAttributes", function (colIndex, colContext) {
					var cellContent = colContext.getObject();
					var path = colContext.getPath();
					var length = that.getModel("oCustomTaskModel").getProperty(path.split("/tableAttributes")[0] + "/tableAttributes").length;
					var cell = contextTM.addCellItem(contextTM, cellContent, path, ExtDatePicker, oCustomTaskModel, that, true);
					if ((length - path.split("/")[9]) === 1) {
						var cont = new sap.m.HBox({
								"renderType": "Bare",
								"alignItems": "Center",
								"justifyContent": "SpaceBetween"
							...
							.addItem(new sap.m.Button({
								...
								visible: true,
								text: "Remove",
								enabled: that.getModel("oCustomTaskModel").getProperty((path.split("/tableAttributes")[0].split("tableContents/")[0]) +
									"tableContents").length > 1 ? true : false,
								tooltip: "Delete Instance",
								press: function (oEvent) {
									var sPath = oEvent.getSource().getBindingContext("oCustomTaskModel").getPath().split("tableContents");
									var data = that.getModel("oCustomTaskModel").getProperty(sPath[0] + "tableContents");
									data.splice(sPath[1].split("/")[1], 1);
									that.getModel("oCustomTaskModel").refresh(true);
								}.bind(that)
							}).addStyleClass("wbAdminMGroupsRemoveBtn"));
						cell = cont;
		...
		addCellItem: function (contextTM, bindingObject, bindingPath, ExtDatePicker, oCustomTaskModel, that, isTable) {
			var item;
			var isVisible = bindingObject.isVisible;
			var isMandatory = bindingObject.isMandatory;
			var isEditable = bindingObject.isEditable;
			if (isTable && bindingObject.dataType === "TEXT AREA") {
				bindingObject.dataType = "INPUT";
			}
			if (bindingObject.dataType === "INPUT") {
				...
				if (bindingObject.dropDownType === "individual") {
					item = new sap.m.MultiComboBox({
						...
						selectionFinish: function (oEvent) {
							...
							contextTM.addResourceList(oEvent, sPath, oCustomTaskModel, bindingObject.dropDownType);
						},
					...
					item = new sap.m.MultiComboBox({
						...
						selectionFinish: function (oEvent) {
							var sPath = oEvent.getSource()._getBindingContext("oCustomTaskModel").getPath();
							contextTM.addMultiSelectValues(oEvent, sPath, oCustomTaskModel);
						},
						selectionChange: function (oEvent) {
							bindingObject.isEdited = 2;
						},
				...
				if (bindingObject.processName === "ProjectProposalDocumentApproval") {
					item.getAggregation("items")[1].setMinDate(this.formatter.wbSetMinDateSubsRule(new Date()));
				}
				...
				item = new sap.m.VBox({
						...
						icon: "sap-icon://add",
						tooltip: that.getView().getModel("i18n").getResourceBundle().getText("UPLOAD_FILE_TEXT"),
						buttonText: that.getView().getModel("i18n").getResourceBundle().getText("ADD_FILES_TEXT"),
						buttonOnly: true,
						fileType: "jpg,jpeg,png,xlsx,ods,xls,docx,zip,ppt,pdf,csv,msg",
						enabled: isEditable,
						placeholder: that.getView().getModel("i18n").getResourceBundle().getText("UPLOAD_FILE_TEXT") + " " + bindingObject.label,
						change: function (oEvent) {
							var sPath = oEvent.getSource()._getBindingContext("oCustomTaskModel").getPath();
							contextTM.handleImageUploadInstanceCreation(oEvent, sPath, oCustomTaskModel);
						}
					}).addStyleClass("wbDefaultButtonClass"))
					.addItem(new sap.m.HBox({
						...
						.addItem(new sap.m.Image({
							...
							press: function (oEvent) {
								contextTM.downloadAttachment(bindingObject.attachmentId, bindingObject.attachmentType, bindingObject.attachmentName,
						...
						.addItem(new sap.m.VBox({
							...
							.addItem(new sap.m.HBox({
								...
								visible: "{path: 'oCustomTaskModel>attachmentName' ,formatter: 'oneapp.incture.workbox.util.formatter.wbShowUploadedImageInstanceCreation'}",
								press: function (oEvent) {
									contextTM.deleteUploadedImage(oEvent, oCustomTaskModel);
								}
							}).addStyleClass("wbAdminMGroupsRemoveBtn")))));
			} else if (bindingObject.dataType === "TEXT AREA") {
		...
		/**
		 * Function to merge subtask instances with the process instance
		 * @public
		 * @param that reference to App.controller.js
		 * @param {Object} the payload object 
		 * @param {ExtDatePicker} Extended ExtDatePicker function
		 ...
		 */
		mergeInstancesTM: function (that, newTaskData, ExtDatePicker, oCustomTaskModel) {
			var subTaskData = oCustomTaskModel.getProperty("/customInstance");
			subTaskData.type = newTaskData.type;
			var subTaskTemplateDto = [];
			var newTaskTemplateDto = [];
			if (subTaskData.listOfProcesssAttributes[0].customAttributeTemplateDto && newTaskData.listOfProcesssAttributes[0].customAttributeTemplateDto) {
				subTaskTemplateDto = subTaskData.listOfProcesssAttributes[0].customAttributeTemplateDto;
				newTaskTemplateDto = newTaskData.listOfProcesssAttributes[0].customAttributeTemplateDto;
			}
			// assign to merged data
			for (var i = 0; i < subTaskTemplateDto.length; i++) {
				for (var j = 0; j < newTaskTemplateDto.length; j++) {
					if (subTaskTemplateDto[i].label.trim() === newTaskTemplateDto[j].label.trim()) {
						if (newTaskTemplateDto[j].processName === "Reimbursementofregisteredcourses" && newTaskTemplateDto[j].processName ===
							"CourseRegistration" && newTaskTemplateDto[j].processName === "ReimbursementofregisteredcoursesDemo" && newTaskTemplateDto[j].processName ===
							"CourseRegistrationDemo") {
							if (!newTaskTemplateDto[j].isEditable) {
								newTaskTemplateDto[j].value = subTaskTemplateDto[i].value;
							}
						} else {
							newTaskTemplateDto[j].value = subTaskTemplateDto[i].value;
						}
			...
			newTaskData.type = "Single Instance";
			newTaskData.actionType = "Submit";
			newTaskData.isEdited = 2;
			oCustomTaskModel.setProperty("/customInstance", newTaskData);
			oCustomTaskModel.refresh();
			this.uploadInstancesTM(that, newTaskData, ExtDatePicker, oCustomTaskModel);
		},
		...
		addGridContentSingleInstanceTM: function (contextTM, bindingObject, bindingPath, ExtDatePicker, oCustomTaskModel, contextGlobal) {
			var that = this;
			var isVisible = true;
			if (bindingObject.dataType === "TABLE") {
				isVisible = false;
			}
			if (bindingObject.processName === "Reimbursementofregisteredcourses") {
				bindingObject.isEditable = true;
			}
			if (bindingObject.processName === "ProjectProposalDocumentApproval") {

				if (bindingObject.label.includes("Available Budget") && (
						bindingObject.dataType === "INPUT" || bindingObject.dataType === "INPUT NUMERIC" || bindingObject.dataType ===
						"INPUT NUMERIC-CALCULATE")) {
					isVisible = false;
					var z = 0;
					...
					if (costCenterValue && costCenterValue.length > 0) {
						for (z = 0; z < costCenterValue.length; z++) {
							if (bindingObject.label.includes(costCenterValue[z])) {
								isVisible = true;
								bindingObject.isEditable = false;
							}
						}
					}
			...
			var isMandatory = bindingObject.isMandatory;
			var item = contextTM.addCellItem(contextTM, bindingObject, bindingPath, ExtDatePicker, oCustomTaskModel, contextGlobal, false);
			var VBox = new sap.m.VBox({
				visible: isVisible,
				justifyContent: "Start",
				renderType: "Bare"
			...
			if (bindingObject.label.toLowerCase() === "description" && bindingObject.processName.toLowerCase() === "standard" && bindingObject.dataType
				.toLowerCase() === "text area") {
				VBox.addStyleClass("wbCreateTaskStretchControl wbCreateTaskDescriptionWrap");
			}
			return VBox;
		},
		...
		addDeleteIconInstanceCreationTM: function (that, oCustomTaskModel) {
			...
			// }
			if (!mainVBox) {
				mainVBox = sap.ui.getCore().byId("ID_INSTANCE_AGGEGRATION_VBOX") || sap.ui.core.UIComponent.getRouterFor(that).getView(
					'oneapp.incture.workbox.view.CreateTask').byId("ID_INSTANCE_AGGEGRATION_VBOX");
			}
		...
		validateMandatoryFieldTM: function (actionType, oCustomTaskModel) {
			...
			for (var i = 0; i < templateDto.length; i++) {
				...
				if (actionType === "Save" || actionType === "SecondSave") {
					...
					for (var j = 0; j < customAttributeTemplate.length; j++) {
						if (customAttributeTemplate[j].processName !== "STANDARD" && customAttributeTemplate[j].isMandatory && customAttributeTemplate[j].dataType !==
							"TABLE") {
		...
		onClickClearTM: function (oCustomTaskModel) {
			...
			for (var j = 0; j < customAttributeTemplate.length; j++) {
				if (customAttributeTemplate[j].key !== "requestId" || customAttributeTemplate[j].key !== "createdAt" || customAttributeTemplate[j]
					.key !==
		...
		deleteUploadedImage: function (oEvent, oCustomTaskModel) {
			...
			oEvent.getSource().getParent().getParent().getParent().getParent().getItems()[0].clear(); //Clear the file uploader
		...
		onImportSelectedRowTM: function (oCustomTaskModel, that, ExtDatePicker) {
			var header = oCustomTaskModel.getProperty("/excelData/header");
				...
				if (oCustomTaskModel.getProperty("/excelDataHeaderOrientation") === "col") {
					...
					for (var m = startRange; m <= endRange; m++) {
						...
						for (j = 0; j < customAttributeTemplateDto.length; j++) {
							for (k = 0; k < rowObject.length; k++) {
								// keys[0]= header
								if (Object.values(rowObject[k])[header] && customAttributeTemplateDto[j].label.toLowerCase().trim() === Object.values(
										rowObject[
			...
			this.uploadInstancesTM(that, oCustomTaskModel.getProperty("/customInstance"), ExtDatePicker, oCustomTaskModel);
			oCustomTaskModel.setProperty("/enableBusyIndicators", false);
		},
							...
							for (var k = 0; k < operatorAttributes.length; k++) {
								if (values[l].attributePath.includes(operatorAttributes[k].key)) {
									if (operatorAttributes[k].label === "+" || operatorAttributes[k].label === "-" || operatorAttributes[k].label === "*" ||
										...
										var tempValues = values[l].attributePath.replace((" '" + operatorAttributes[k].label + "' "), (" " + operatorAttributes[k]
											.label +
											" "));
		...
		generateTaskTemplateDropdown: function (oDefaultDataModel) {
			...
			for (var i = 0; i < data.length; i++) {
				...
				for (var j = 0; j < data.length; j++) {
					if (data[i].templateId !== data[j].templateId) {
						var sourceDropdown = {
							"text": data[j].eventName,
							"key": data[j].templateId,
							"isEdited": data[j].isEdited
						};
						taskSourceDropdown.push(sourceDropdown);
					}
				}
				var defaultSourceDropdown = {
					"text": "None",
					"key": ""
				};
				taskSourceDropdown.push(defaultSourceDropdown);
				data[i].sourceDropdown = taskSourceDropdown;
			}
			var defaultObj = {
				"text": "None",
				"key": ""
		...
		addExistingAttributestoTaskTM: function (that, oEvent, oDefaultDataModel) {
			...
			for (var j = 0; j < listItems.length; j++) {
				var oItem = listItems[j].getObject();
				// if (oItem.dataType === "DROPDOWN") { // for dropdowns send isEdited in dropDownValues as 2 
				// 	if (!oItem.dropDownValues) {
				// 		oItem.dropDownValues = [];
				// 	}
				// 	if (oItem.dropDownValues.length > 0) {
				// 		for (var i = 0; i < oItem.dropDownValues.length; i++) {
				// 			oItem.dropDownValues[i].isEdited = 2;
				// 		}
				// 	}
				// }
		...
		deleteTaskLevelFnTM: function (oEvent, that, oDefaultDataModel) {
			...
			if (oDefaultDataModel.getProperty("/setEnabled") === false) {
				if (data[index].isEdited === 2) {
					...
					data[index].isEdited = 3;
					data.splice(index, 1); //on updation
				}
			} else {
				data.splice(index, 1);
			}
		...
		createWorkFlowTaskValidationTM: function (that, oDefaultDataModel, calculativeCheck) {
			var validationCheck;
				...
				if (workFlowTaskData[i].customAttributes) {
					...
					for (var j = 0; j < taskAttributes.length; j++) {
						...
						if (taskAttributes[j].dataType === "DROPDOWN" && taskAttributes[j].isRunTime === false) {
							var values = [];
							if (taskAttributes[j].dropDownValues) {
								values = taskAttributes[j].dropDownValues;
							}
				...
				if (teamDto) {
					for (var i = 0; i < teamDto.length; i++) {
						...
						if ((teamDto[i].eventName === "") || ((teamDto[i].taskNature === "User Based") && (teamDto[i].individual.length === 0 &&
									teamDto[i].group.length === 0) && teamDto[i].runTimeUser === 0 && teamDto[i].ownerSelectionRules.length ===
								0)) {
							validationCheck = 1;
							break;
						}
						...
						for (var z = 0; z < workFlowCustomData.length; z++) {
							...
							if (workFlowCustomData[z].dataType === "DROPDOWN" && workFlowCustomData[z].isRuntime !== true) {
							...
							} else if (workFlowCustomData[z].dataType === "INPUT NUMERIC-CALCULATE") {
								var operatorAttributes = oDefaultDataModel.getProperty("/processLevelOperatorAttributes");
								var values = workFlowCustomData[z].attributePath;
								for (var k = 0; k < operatorAttributes.length; k++) {
									if (values.includes(operatorAttributes[k].label)) {
										if (operatorAttributes[k].label === "+" || operatorAttributes[k].label === "-" || operatorAttributes[k].label === "*" ||
											operatorAttributes[k].label === "/") {
								...
								workFlowCustomData[z].attributePath = values;
							}
						}
					}
				}
					...
					if (customData[j].dataType === "DROPDOWN" && customData[j].isRunTime === false) {
						...
						if (values) {
							for (var k = 0; k < values.length; k++) {
								...
								values[k].processName = customData[j].key;
								if (!values[k].label) {
									validationCheck = 1;
									break;
								}
						...
						var calculativeCheck = 1;
						this.createWorkFlowTaskValidationTM(that, oDefaultDataModel, calculativeCheck);
					}
				}
			}
		...
		onProcessListItemSelectTM: function (that, oEvent, url, oDefaultDataModel, oAppModel) {
			...
			that.doAjax(url, "GET", null, function (oData) {
				...
				for (var j = 0; j < teamDetailDto.length; j++) {
					...
					var values = teamDetailDto[j].customAttributes;
					for (var l = 0; l < values.length; l++) {
						if (values[l].dataType === "INPUT NUMERIC-CALCULATE") {
							for (var k = 0; k < operatorAttributes.length; k++) {
		...
		setServerStatusTimer: function (that) { // timer added to overall application
			...
			var interval = setInterval(function () {
				...
				that.doAjax(url, "GET", null, function (data) {
					if (data.message === "FAILURE") {
						that._createConfirmationMessage(title, msg, "Warning", "OK", "OK", false, function () {}, function () {
							location.reload();
							clearInterval(interval);
						}.bind(that));
					}
				}.bind(this), function (oError) {
					if (oError.getParameter("statusCode") >= 400 && oError.getParameter("statusCode") <= 599) {
						that._createConfirmationMessage(title, msg, "Warning", "OK", "OK", false, function () {}, function () {
							location.reload();
							clearInterval(interval);
						}.bind(that));
