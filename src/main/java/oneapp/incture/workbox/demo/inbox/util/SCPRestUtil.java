...
//			if (appToAppSSOHeader != null && httpRequestBase != null) {
//				httpRequestBase.addHeader(appToAppSSOHeader.getName(), appToAppSSOHeader.getValue());
//			}
			if (!ServicesUtil.isEmpty(xCsrfToken) && !xCsrfToken.equalsIgnoreCase("fetch") && httpRequestBase != null)
				// httpRequestBase.addHeader("x-csrf-token", xCsrfToken);
				httpRequestBase.addHeader("X-CSRF-Token", xCsrfToken);
...
//	public static AuthenticationHeader refreshAppToAppSSOHeader(String requestURL) {
//		Context ctx;
//		AuthenticationHeader appToAppSSOHeader = null;
...
//				}
				...
				if (httpRequestBase != null && !ServicesUtil.isEmpty(xCsrfToken) && !xCsrfToken.equalsIgnoreCase("fetch"))
					httpRequestBase.addHeader("X-CSRF-Token", xCsrfToken);
				if(httpRequestBase != null)
					httpRequestBase.addHeader("accept", contentType);
