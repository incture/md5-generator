package oneapp.incture.workbox.demo.zohoExpense.dao;
...
@Repository
//@Transactional
public class ZohoExpenseTaskEventsDao {
	...
	public void updateTaskStatus(String processName) {
		...
		Query customAttributesQuery = this.getSession().createSQLQuery("update  task_events set status = 'CANCELLED'"
				+ " where status='READY' and  PROCESS_ID = '" + processName + "'");
	...
	public void completeTaskStatus(String taskId) {
		Session session = sessionFactory.openSession();
        Transaction tx = session.beginTransaction();
		Query customAttributesQuery = this.getSession()
				.createSQLQuery("update  task_events set status = 'COMPLETED'  where   event_id = '" + taskId + "'");
		customAttributesQuery.executeUpdate();
		session.flush();
		session.clear();
		tx.commit();
	...
	public void updateAssignee(String taskId, String pUserId) {
		Session session = sessionFactory.openSession();
        Transaction tx = session.beginTransaction();
		Map<String, UserIDPMappingDto> userList = zohoService.fetchUsers(1);
		String taskOwnerDisplayName = userList.get(pUserId).getUserFirstName() + " "
				+ userList.get(pUserId).getUserLastName();
