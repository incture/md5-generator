...
import oneapp.incture.workbox.demo.adminConsole.dao.ReportAgingDao;
import oneapp.incture.workbox.demo.adminConsole.dto.AdminControlDto;
import oneapp.incture.workbox.demo.adminConsole.dto.AgingRangeConfigDto;
import oneapp.incture.workbox.demo.adminConsole.dto.ReportAgingDto;
import oneapp.incture.workbox.demo.adminConsole.dto.ReportDto;
...
public class AdminControlFacade implements AdminControlFacadeLocal {
	...
	public ResponseMessage createUpdateDataAdmin(AdminControlDto adminControlDto) {
		...
		ResponseMessage responseDto = new ResponseMessage();
		System.err.println("[WBP-Dev]adminControlDto" + adminControlDto.toString());
					...
					for (WorkloadRangeDto dto1 : workloadRangeDtos) {
						try {
							if (!workloadRangeDao.updateWorkloadRange(dto1).equals(PMCConstant.SUCCESS)) {
								workloadRangeDao.saveOrUpdate(dto1);
							}
						} catch (Exception eWorkloadRange) {
							System.err.println("[WBP-Dev][PMC]" + eWorkloadRange.getLocalizedMessage());
						}
					}
				}
				// if (!ServicesUtil.isEmpty(reportAgingDtos)) {
