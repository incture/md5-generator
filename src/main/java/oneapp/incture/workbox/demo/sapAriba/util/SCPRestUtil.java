...
@Component("scpAribaRestUtil")
public class SCPRestUtil {
...
//					}
					...
					for (Header header : headers) {
						...
						if (header.getName().equalsIgnoreCase("x-csrf-token")) {
							System.err.println(
									"[WBP-Dev]SCPRestUtil.callRestService() x-csrf-token is : " + header.getValue());
...
//				}
	...
	/*
	 * public static AuthenticationHeader refreshAppToAppSSOHeader(String
	 * requestURL) { Context ctx; AuthenticationHeader appToAppSSOHeader = null;
	 * AuthenticationHeaderProvider authHeaderProvider; try { ctx = new
	 * InitialContext(); authHeaderProvider = (AuthenticationHeaderProvider)
	 * ctx.lookup("java:comp/env/AuthHeaderProvider"); appToAppSSOHeader =
	 * authHeaderProvider.getAppToAppSSOHeader(requestURL); } catch (Exception ex) {
	 * logger.error("Exception while fetching auth Header Provider : " +
	 * ex.getMessage()); } return appToAppSSOHeader; }
	 */
...
//				if (!ServicesUtil.isEmpty(appToAppSSOHeader))
//					System.err.println("[WBP-Dev][appToAppSSOHeader][1]" + appToAppSSOHeader.getName() + " "
...
//						}
						...
						httpRequestBase.addHeader("SAP-Connectivity-ConsumerAccount",
								PropertiesConstantsStatic.CONN_TENANT);
						httpResponse = httpClient.execute(httpRequestBase);
			...
			if (!ServicesUtil.isEmpty(httpClient)) {
				...
				try {
					if (xCsrfToken != null && xCsrfToken.equalsIgnoreCase("fetch")) {
						...
						httpResponse = httpClient.execute(httpRequestBase);
						System.err.println(
								"[WBP-Dev]chatbot [callRestService][x-csrf-token][httpResponse]" + httpResponse);
						Header[] headers = httpResponse.getAllHeaders();
						for (Header header : headers) {

							if (header.getName().equalsIgnoreCase("x-csrf-token")) {
								System.err.println("[WBP-Dev]chatbot[callRestService][header.getName()]"
										+ header.getName() + ": header.getValue()" + header.getValue());
								xCsrfToken = header.getValue();
							}
						}
					}
				} catch (IOException e) {
					System.err.println("[WBP-Dev]chatbot [callRestService] Exception : " + e.getMessage());
				}
				...
				if (method.equalsIgnoreCase(PMCConstant.HTTP_METHOD_GET)) {
					...
					if (!ServicesUtil.isEmpty(entity)) {
						try {
							...
							logger.error(" chatbot [callRestService] Input UnsupportedEncodingException : "
									+ e.getMessage());
						}
						((HttpPost) httpRequestBase).setEntity(input);
					}
